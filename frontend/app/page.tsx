...
function ToolbarIcon({ type }: { type: "history" | "chat" }) {
  return (
        ...
        <PipelineStepper
          ...
          actions={
            <>
              ...
              <button type="button" onClick={() => setChatOpen((value) => !value)} className={`flex h-11 items-center gap-2 rounded-lg px-2.5 text-[11px] font-medium transition focus-visible:outline-2 focus-visible:outline-ember-500 ${chatOpen ? "bg-ember-100 text-ember-600" : "text-ink-500 hover:bg-paper-200 hover:text-ink-900"}`} aria-label={chatOpen ? "Hide conversation" : "Open conversation"} aria-pressed={chatOpen}><ToolbarIcon type="chat" /><span className="hidden xl:inline">Conversation</span></button>
            </>
          }
        />

        <div className="analysis-workspace min-h-0 flex-1">
          <div className="analysis-visual-grid h-full min-h-0">
            <section className="analysis-map-pane relative min-h-[20rem] overflow-hidden border-paper-300" aria-label="Map result">
              <MapView resolved={resolved} layers={session.layers} rasters={session.rasters} activeLayerIds={activeLayerIds} onLayerSelect={selectLayer} />
            </section>
